...
class Linear(DQN):
    ...
    def get_q_values(self, state: torch.Tensor, network: str = "q_network"):
        ...
        """
        b = state.size(dim=0)
        out = torch.zeros([b, self.env.num_actions()])

        ##############################################################
        ################ YOUR CODE HERE - 3-5 lines ##################
        work_net = self.q_network if network == 'q_network' else self.target_network
        lst_tens = [torch.unsqueeze(work_net(torch.flatten(state[i])), 0) for i in range(b)]
        
        out = torch.cat(lst_tens)
    ...
    def calc_loss(
        ...
        ##############################################################
        ##################### YOUR CODE HERE - 3-5 lines #############
        b = q_values.size(dim=0)
        q_samp = torch.zeros(b)
        
        max_val = torch.max(target_q_values,-1).values
        for i in range(b):
            q_samp[i] = rewards[i] if done_mask[i] else rewards[i]+gamma*max_val[i]
        # q_samp = torch.where(done_mask,rewards,r_alt)
        actions_stack = F.one_hot(actions.to(torch.int64), num_actions)
        q = torch.sum(torch.multiply(q_values,actions_stack),1)
        self.loss = F.mse_loss(q_samp,q)
        return self.loss
        ##############################################################
        ######################## END YOUR CODE #######################
