...
class NatureQN(Linear):
    ...
    def initialize_models(self):
        ...
        inp_size = n_channels * self.config.state_history
        self.q_network = nn.Sequential(nn.Conv2d(inp_size, 16, 3), nn.ReLU(), nn.Flatten(), nn.Linear(1024, 128), nn.ReLU(), nn.Linear(128,num_actions))
        self.target_network = nn.Sequential(nn.Conv2d(inp_size, 16, 3), nn.ReLU(), nn.Flatten(), nn.Linear(1024, 128), nn.ReLU(), nn.Linear(128,num_actions))
        ##############################################################
        ######################## END YOUR CODE #######################
    ...
    def get_q_values(self, state, network):
        ...
        state_p = state.permute((0, 3, 1, 2))
        out = work_net(state_p)
        ##############################################################
        ######################## END YOUR CODE #######################
        return out
